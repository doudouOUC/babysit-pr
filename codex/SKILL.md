...
After the model **actually fixes something and the fix is pushed** to the PR branch, send a DingTalk notification via the helper script. This keeps the user informed of autonomous fixes happening in background watches without forcing them to watch the terminal.
